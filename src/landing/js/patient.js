...
function createAccordionItem(parentElem, parentId, number, header, text, changable = false) {
    /*
    <div className="accordion-item">
        ...
        <div id="collapseOne" className="accordion-collapse collapse"
             aria-labelledby="headingOne" data-bs-parent="#accordionAnalyzes">
            <div className="accordion-body">
                Група крові такато, кількість єретроцитів така-то, и тд. Група крові такато, кількість єретроцитів
                така-то, и тд.Група крові такато, кількість єретроцитів така-то, и тд.Група крові такато, кількість
                єретроцитів така-то, и тд.Група крові такато, кількість єретроцитів така-то, и тд.Група крові такато,
                кількість єретроцитів така-то, и тд.Група крові такато, кількість єретроцитів така-то, и тд.Група крові
                такато, кількість єретроцитів така-то, и тд.Група крові такато, кількість єретроцитів така-то, и
                тд.Група крові такато, кількість єретроцитів така-то, и тд.Група крові такато, кількість єретроцитів
                така-то, и тд.
            </div>

        </div>
    </div>*/
    let accordionItemContainer = document.createElement("div");
    accordionItemContainer.className = "accordion-item";

    let accordionHeaderContainer = document.createElement("h2");
    let  accordionHeaderContainer_id = `${parentId}Elem${number}HeaderContainer`;
    accordionHeaderContainer.id = accordionHeaderContainer_id;
    accordionHeaderContainer.className = "accordion-header";

    let accordionBodyContainer = document.createElement("div");
    let accordionBodyContainer_id = `${parentId}Elem${number}BodyContainer`;
    accordionBodyContainer.id = accordionBodyContainer_id;
    accordionBodyContainer.className = "accordion-collapse collapse";
    accordionBodyContainer.setAttribute("aria-labelledby",`${accordionHeaderContainer_id}`);
    accordionBodyContainer.setAttribute("data-bs-parent",`#${parentId}`);

    let accordionHeaderButton = document.createElement("button");
    accordionHeaderButton.className = "accordion-button collapsed";
    accordionHeaderButton.type = "button";
    accordionHeaderButton.setAttribute("data-bs-toggle","collapse");
    accordionHeaderButton.setAttribute("data-bs-target",`#${accordionBodyContainer_id}`)
    accordionHeaderButton.setAttribute("aria-expanded",`false`)
    accordionHeaderButton.setAttribute("aria-controls",`${accordionBodyContainer_id}`)
    accordionHeaderButton.innerText = header;

    let accordionBody = document.createElement("div");
    accordionBody.className = "accordion-body";
    accordionBody.innerText = text;

    accordionBodyContainer.appendChild(accordionBody);
    accordionHeaderContainer.appendChild(accordionHeaderButton);

    accordionItemContainer.appendChild(accordionHeaderContainer);
    accordionItemContainer.appendChild(accordionBodyContainer);
    parentElem.appendChild(accordionItemContainer);
    console.log('done');
}
//createAccordionItem(document.getElementById("accordionAnalyzes"),"accordionAnalyzes",3,"test3","text");


let patients_id = getCookie("patient_id");
console.log(patients_id);
let exist_patient = true;
if(patients_id==0)
    exist_patient = false;
else{
    postData('/patient/getPatientData', {patient_id: patients_id}).then((data)=>{
        document.getElementById('inputFIO').value = data.Name;
...
function Change_name(element){
    if(exist_patient) {
        ...
        postData('/patient/changePatientData', change_data);
    }
    else {
        ...
        let doctor_id = getCookie('id');
        let patient_data = {doctor: doctor_id, name: name, age: +age, height: +height, weight: +weight};
        console.log(patient_data);
        postData('/patient/createPatient', patient_data).then((data) => {
            patients_id = data.id;
            exist_patient = true;
        });
    }
}
function Change_age(element){
    if (exist_patient) {
        let change_data = {
            patient_id: patients_id,
            change: 'age',
...
function getImagesOriginals() {
    if (exist_patient) {
        let UserData = {
            patient_id: patients_id,
            doctor_id: getCookie('id')
        };
        postData('/patient/getOriginals', UserData)
            .then((filenames) => {
                filenames.forEach((filename)=>{
                    console.log(filename);
                    let image = document.createElement("img");
                    image.classList.add('add-patient-images');
                    image.src = `${getCookie('id')}/${patients_id}/originals/${filename}`;
                    document.getElementById("imgTable").appendChild(image);
                });
            });
        ...
        postData('/patient/getAnalyzed', UserData)
            .then((data) => {
                data.forEach((file_data)=>{
                    ...
                    analyz_container.appendChild(span);
                    document.getElementById("workImgs_container").appendChild(analyz_container);
                });
            });
...
getImagesAnalyzed();
function load_file_original(element){
    let crop_container = document.getElementById('crop-container');
    let cropper_image = document.getElementById('image');
    let uploaded_file = element.files[0];
    cropper_image.src = URL.createObjectURL(uploaded_file);
    crop_container.style.display = "block";
    let cropper = new Cropper(cropper_image, {
        aspectRatio: 0,
    });
    console.log(uploaded_file);
    let formImageUploadData = new FormData();
    formImageUploadData.append('file', uploaded_file);
    formImageUploadData.append('type', 'originals');
    formImageUploadData.append('patient_id', patients_id);
    formImageUploadData.append('doctor_id', getCookie('id'));
    postImage('/patient/upload-photo', formImageUploadData).then((response)=>{
        console.log(response);
        let image_not_cropped = document.createElement("img");
        image_not_cropped.classList.add('add-patient-images');
        image_not_cropped.src = URL.createObjectURL(element.files[0]);
        document.getElementById("imgTable").appendChild(image_not_cropped);
    });
    document.getElementById('crop-butt').addEventListener('click',()=>{


        let image = document.createElement("img");
        image.classList.style = "";
        image.src = cropper.getCroppedCanvas().toDataURL("image/png");
        work_image = cropper.getCroppedCanvas().toDataURL("image/png");
        let analyz_container = document.createElement("div");
        analyz_container.style = "width: 80%; height: 300px; display:flex";
        let span = document.createElement("span");
        analyz_container.appendChild(image);
        ...
        crop_container.style.display = "none";
        cropper.destroy();
        console.log("yes");
...
function old_load_file(element){
        let crop_container = document.getElementById('crop-container');
        ...
        document.getElementById('crop-butt').addEventListener('click',()=>{
            ...
            cropped_img.src = cropper.getCroppedCanvas().toDataURL("image/png");
            all_images.push(cropper.getCroppedCanvas().toDataURL("image/png"));
            work_image = cropper.getCroppedCanvas().toDataURL("image/png");
            let analyze_photo_container = document.getElementById("workImg_container");
            analyze_photo_container.removeChild(analyze_photo_container.firstChild);
            cropped_img.classList.add("analyze-photo");
            analyze_photo_container.appendChild(cropped_img);
            ...
            console.log("yes");
            let img_data = {
                image: all_images[all_images.length-1],
                patient_id: patients_id,
                doctor_id: getCookie('id')
            };

            postData('/patient/upload-photo', img_data);
...
document.getElementById('analyze-button').addEventListener('click',()=>{
    if(work_image!=0)
    {
        let task_drop = +(document.getElementById("typeSelect").value);
        let send_analyze_data = {
            id: getCookie('id'),
            patient_id: patients_id,
            task: task_drop,
            image: work_image
        };
        postData('/patient/analyze', send_analyze_data)
            .then((data) => {
